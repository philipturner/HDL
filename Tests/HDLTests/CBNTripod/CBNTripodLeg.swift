...
struct CBNTripodLeg: CBNTripodComponent {
  ...
  init() {
    ...
    compilationPass0()
    XCTAssertEqual(topology.atoms.count, 10)
    XCTAssertEqual(topology.bonds.count, 0)
    
    compilationPass1()
    bondRecord = createBondRecord()
    XCTAssertEqual(topology.atoms.count, 10)
    XCTAssertEqual(topology.bonds.count, 10)
    XCTAssertEqual(bondRecord.keys.count, 3)
    expectedRecord = [
      SIMD2(6, 6): 6,
      SIMD2(6, 7): 1,
      SIMD2(6, 9): 3,
    ...
    compilationPass2()
    bondRecord = createBondRecord()
    XCTAssertEqual(topology.atoms.count, 15)
    ...
    compilationPass3()
    bondRecord = createBondRecord()
    XCTAssertEqual(topology.atoms.count, 20)
    XCTAssertEqual(topology.bonds.count, 20)
    XCTAssertEqual(bondRecord.keys.count, 7)
    expectedRecord = [
      SIMD2(1, 6): 4,
      SIMD2(1, 7): 1,
      SIMD2(1, 14): 3,
    ...
    compilationPass4()
    bondRecord = createBondRecord()
    XCTAssertEqual(topology.atoms.count, 14)
    XCTAssertEqual(topology.bonds.count, 14)
    XCTAssertEqual(bondRecord.keys.count, 6)
    expectedRecord = [
      SIMD2(1, 6): 1,
      SIMD2(1, 7): 1,
      SIMD2(6, 6): 7,
